...
setwd("D:/IDUP/1_Analyse_Sequence/A envoyer/AL_Seance3/TP3_Format")
...
# Quels sont les déterminants de l'accès à la propriété ?
...
# On garde que la dernière ligne pour chaque individu
# On prend le format long car c'est le plus précis
Enq <- A %>% 
  group_by(IDENT) %>% 
  filter(row_number()==n()) %>% 
